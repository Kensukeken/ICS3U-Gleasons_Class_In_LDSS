...
std::cin >> userAnswer; //I was initially uncertain about how to resolve the issue, I took the initiative to conduct some research and also referred to tutorials on YouTube, which helped me gain a better understanding of the problem and ultimately led me to a solution

for (int i = 0; i < userAnswer.length(); i++) {
  userAnswer[i] = tolower(userAnswer[i]);//This code converts all characters in the string variable "userAnswer" to lowercase by iterating over each character using a "for" loop and applying the "tolower" function.
}
...
printf("Let's start the quiz!\n");
  ...
  printf("Sorry, you can't take the quiz.\n");
  }
} //This code asks the user if they are ready for a quiz, and if they want to see the questions before answering. It then asks a series of questions and checks the user's answers, displaying the result at the end.
